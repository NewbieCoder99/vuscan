module Md5
	...
	class Cracker
		...
		def if_1(password)
			...
			fsize.each do |l|
				...
				x_Array.each do |x|
					...
					LOG.new.create("cache/1/#{l}.txt","#{passEnc}		#{msg}")
				end
			end
			...
			fsize.each do |l|
				...
				x_Array.each do |x|
					y_Array.each do |y|
						...
						LOG.new.create("cache/2/#{l}.txt","#{passEnc}		#{msg}")
					end
				end
		...
		def if_3(password)
			...
			fcache 	= "#{$cache_path}/3_char.txt"
			files 	= File.readlines($tmp_f)
			fsize 	= 0..files.size.to_i - 1
			check	= LOG.new.word_exists_in_file(fcache,password)

			if LOG.new.word_exists_in_file(fcache,password) == true
				files 	= open(fcache).read
				x1 	= files.split(password)
				x2	= x1[1].split("\n")
				puts "\n- Passowrd found : #{password}".white + x2[0].yellow + "\n\n"
			else
				fsize.each do |l|
					...
					x_Array.each do |x|
						y_Array.each do |y|
							z_Array.each do |z|
								...
								LOG.new.create("cache/3/#{l}.txt","#{passEnc}		#{msg}")
							end
						end
					end
					...
					y_Array.each_with_index do |y|
						z_Array.each_with_index do |z|
							x1_Array.each_with_index do |x1|
								...
								LOG.new.create("cache/4/#{l}.txt","#{passEnc}		#{msg}")
							end
						end
					end
				end
						...
						z_Array.each_with_index do |z|
							x1_Array.each_with_index do |x1|
								y1_Array.each_with_index do |y1|
									...
									passEnc = Digest::MD5.hexdigest("#{msg}")
									print "- #{passEnc} #{msg} 	#{l + 1}/#{files.size.to_i}\n"
								end # End Loop y1File.open("foo.msh","wb") {|f|  Marshal.dump(myobj, f) }
							end # End Loop x1
						end # End Loop z
					end # End Loop y 
				end # End Loop x
